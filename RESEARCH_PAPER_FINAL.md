...
In 2016, an investor constructs a systematic trading strategy targeting India's small-cap stocks. Following standard practice, she backtests the strategy using the current constituents of the NIFTY Smallcap 250 index—the 252 stocks that comprise the index today. The results are impressive: a Sharpe ratio of 1.160 and annualized returns of 26.17%. Confident in these metrics, she allocates capital.
...
Had she tested on the complete historical universe of 1,437 stocks, her Sharpe ratio would have been 1.063 (not 1.160), and her annualized returns 21.23% (not 26.17%). While still respectable, these represent a 23.2% overstatement of performance—the difference between an excellent strategy and a mediocre one.
...
```
Market Cap Proxy(i) = Price(i) × Volume(i)
```
...
For each quarter-end date from March 31, 2016 to September 30, 2025 (39 quarters), I:
...
Rank all stocks by MktCapProxy in descending order:

```
Rank(i,t) = rank(MktCapProxy(i,t))
```
...
```
SmallCap250(t) = {stocks with rank 151 to 400}
```
...
| **Volatility (annualized)** | 22.6% | 20.0% | **+2.6pp** | **+13.0%** |
...
**This is the "smoking gun" evidence**: concrete examples of stocks that dramatically underperformed, exited the index, and would be systematically excluded from survivor-biased backtests. The exclusion of just these ten stocks contributes significantly to the overall +4.93pp bias.
...
**In summary, survivor-only backtesting overstates Indian small-cap returns by 23.2% (4.93 percentage points annually) and Sharpe ratios by 9.1% (0.097 points), driven by the systematic exclusion of 1,185 stocks that exited the index over nine years. This bias persists across all performance metrics and is both economically significant and statistically robust. The next section discusses implications and robustness.**
...
The 4.93pp annual bias I document in Indian small-caps substantially exceeds the 1-2% biases found in prior research on U.S. mutual funds (Brown et al., 1995; Elton et al., 1996) and U.S. equities (Shumway, 1997). Three factors explain this:
...
**Result**: Bias increases to 5.2pp (-50% assumption), 5.8pp (-75%), or 6.5pp (-100%). The baseline result (4.93pp) is conservative—if delisted stocks experienced worse final losses than observed, the bias would be even larger.
